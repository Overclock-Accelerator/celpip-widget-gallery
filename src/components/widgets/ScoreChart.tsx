...
export type ScoreEquivalencyTableProps = {
  heading?: string;
  subheading?: string;
  rows?: ScoreRow[];
};
...
export function ScoreEquivalencyTable({
  heading = "Score Equivalency Chart",
  subheading = "CELPIP · CLB · CEFR · DHA (Australia)",
  rows = defaultRows,
}: ScoreEquivalencyTableProps = {}) {
  return (
    <div className="rounded-xl bg-white border border-gray-200 overflow-hidden">
      <div className="bg-[#0B2341] p-6">
        <h3 className="font-heading text-xl font-bold text-white">{heading}</h3>
        {subheading && <p className="text-gray-400 text-sm mt-1">{subheading}</p>}
      </div>
